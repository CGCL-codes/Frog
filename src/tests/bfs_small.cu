...
int main(int argc, char *argv[])
{
	...
	float gpu_time_execute = 0, gpu_time = 0, gpu_time_edge = 0;
	...
	while(iter-- && m_stop[0])
	{
		...
		for(j = 0; j < chunk; j++)
		{
            ...
            BFS_E<<<blockv, 256>>>(d_value, d_all_edge[j], num[j], i, d_stop);
			//BFS_V_S<<<blockv, 256>>>(d_value, d_all_vertex[j], d_all_edge[j], n_vertex[j], i, d_stop);
            cudaEventRecord(stop2, 0);
            cudaEventSynchronize(stop2);
            cudaEventElapsedTime(&gpu_time_edge, start2, stop2);
            gpu_time_execute += gpu_time_edge;
		}
		i++;
	...
	std::cout <<"Total GPU proc time is: " << gpu_time_execute <<std::endl;
	...
	for(unsigned int v = 0; v < v_num; v++)
	{
		...
		if(v < 10)
		{
			std::cout << v <<" vertex bfs value is " << m_value[v] << std::endl;
		}
